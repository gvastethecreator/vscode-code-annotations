...
`media/source/code-annotations-imagegen-raw.png` retains the generated isolated code-line concept. `media/source/code-annotations-imagegen.png` is the accepted crisp vectorized semi-3D native-alpha raster with five open code lines, one integrated annotation marker, controlled gradient depth, and no editor or browser tile. `media/icon.png` is a direct alpha-preserving 256×256 downsample; no SVG reinterpretation is allowed. `media/preview.png` is a tightly cropped native-alpha editor capture from the final installed VSIX, showing the configured annotation tokens on synthetic TypeScript code.
